...
2. Use `language: el`, `translationKey: cars-vs-llms`, and the desired Greek `slug` in frontmatter. Keep the `translationKey` identical to the English entry at `src/content/articles/en/cars-vs-llms.mdx`.
...
The dynamic article route discovers the new entry automatically, emits `/el/articles/{slug}/`, pairs it with English through `translationKey`, enables the language switcher, and adds reciprocal `hreflang` links and the page to the sitemap.
